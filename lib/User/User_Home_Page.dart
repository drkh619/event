...
class _User_HomePageState extends State<User_HomePage> {
  ...
  Future<List<dynamic>> getAllEvents() async {
    String url = "https://$ip_address/Event_Management/User/allevent.php";
    final response = await http.get(Uri.parse(url));
    // return json.decode(response.body);
    if (response.statusCode == 200) {
      List<dynamic> allEvents = json.decode(response.body);
  ...
  Future<List<dynamic>> checkToken(String token) async {
    String apiEndpoint = 'https://$ip_address/Event_Management/User/checkToken.php?token=$token';
...
Future<bool> checkToken(String token) async {
  try {
    String apiEndpoint = 'http://192.168.18.52/Event_Management/User/privateevent.php?token=$token';
    final response = await http.get(Uri.parse(apiEndpoint));
